...
# TASK 2:
# Add a callback function for `site-drdown` as input, `success-pie-chart` as output
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(names=['Success', 'Failed'], values=[success_count, failed_count], title=f'Success/Failure for {entered_site}')
    return fig
...
def success_payload_scatter_chart(entered_site, payloadd):
    if entered_site == 'ALL':
